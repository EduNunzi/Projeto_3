...
void Exportar_Por_Prioridade(struct STarefas tarefas[], int numero_de_tarefas) {
    ...
    if (Exportacao_de_Tarefas > 0) {
        printf("Exportacao conclida. O arquivo 'Tarefas_Prioridade.txt' foi gerado.\n", Prioridade_da_Tarefa);
    } else {
...
void Exportar_Por_Prioridade_E_Categoria(struct STarefas tarefas[], int numero_de_tarefas) {
    ...
    int Priorida_da_Tarefa;
    printf("Digite a prioridade que deseja filtrar: ");
    scanf("%d", &Priorida_da_Tarefa);
    ...
    FILE *arquivo = fopen("Tarefas_Prioridade_e_Categoria.txt", "w");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo de tarefas por prioridade para escrita.\n");
        return;
    }
    ...
    for (int i = 0; i < numero_de_tarefas; i++) {
        if ((strcmp(tarefas[i].categoria, Categoria_da_Tarefa) == 0) && tarefas[i].prioridade == Priorida_da_Tarefa){
            fprintf(arquivo, "Prioridade: %d\n", tarefas[i].prioridade);
            fprintf(arquivo, "Descricao: %s", tarefas[i].descricao);
            fprintf(arquivo, "Categoria: %s", tarefas[i].categoria);
            fprintf(arquivo, "Estado: %s\n", tarefas[i].estado);
    ...
    if (Exportacao_de_Tarefas > 0) {
        printf("Exportacao conclida. O arquivo 'Tarefas_Prioridade_e_Categoria.txt' foi gerado.\n",Priorida_da_Tarefa, Categoria_da_Tarefa);
    } else {
        printf("Nao ha tarefas com a prioridade %d e categoria %s para exportacao.\n",Priorida_da_Tarefa, Categoria_da_Tarefa);
    }
}